...
export function useProjectEvents(showToast: (msg: string, type?: 'success' | 'warn' | 'info' | 'error') => void) {
  ...
  const handleExtractEventsWithAi = useCallback((activeProjectTitle?: string | unknown) => {
    const projectTitle = typeof activeProjectTitle === 'string' && activeProjectTitle.trim() ? activeProjectTitle : '프로젝트';
      ...
      const extracted: ProjectEvent[] = [
        {
          ...
          title: `[AI 추출] '${projectTitle}' 1차 요구사항 정의`,
