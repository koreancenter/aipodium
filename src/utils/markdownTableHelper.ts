...
/**
 * Empties the content of currently selected cells in a markdown table.
 * If range selection is present, clears all cells overlapping the selection range.
 * If no range is selected, clears the single cell at the current cursor position.
 */
export function clearTableCells(
  tableInfo: MarkdownTableInfo,
  selectionStart: number,
  selectionEnd: number
): { formatted: string; newCursorOffset: number } {
    ...
    for (let r = 0; r < tableInfo.lines.length; r++) {
      ...
      if (r !== 1) {
        ...
        for (let c = 0; c < pipeIndices.length - 1; c++) {
          ...
          // Check overlap with [selectionStart, selectionEnd]
      ...
      lineOffset += lineLen + 1; // +1 for '\n'
    }
  }

  const formatted = formatMarkdownTable(newHeaders, tableInfo.alignments, newRows);
  const newCursorCol = tableInfo.cursorColIndex;
  const newCursorRow = tableInfo.cursorRowIndex;
  const newCursorOffset = tableInfo.startOffset + findCellOffsetInTable(formatted, newCursorRow, newCursorCol);
