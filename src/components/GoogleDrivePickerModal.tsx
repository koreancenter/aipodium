  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-slate-200 tracking-tight flex items-center gap-2">
                구글 드라이브 파일 탐색기
                <span className="text-[0.625rem] font-medium px-2 py-0.5 rounded bg-emerald-950/60 border border-emerald-700/50 text-emerald-400">
                  보안 모드 (전용 파일 접근)
                </span>
              </h2>
            </div>
            <p className="text-xs text-slate-400">
              계정 로그인 상태와 무관하게 구글 드라이브 파일을 안전하게 열고 저장합니다.
            </p>
          </div>
            ...
            {authStatus === 'connected' ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-2.5 py-1 text-xs text-rose-400 hover:text-rose-300 hover:bg-rose-950/40 rounded border border-rose-900/50 transition flex items-center gap-1 cursor-pointer"
                  title="구글 드라이브 연동 해제"
                >
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 rounded transition flex items-center gap-1.5 cursor-pointer shadow-xs"
              >
                <HardDrive className="w-3.5 h-3.5" />
                <span>{isConnecting ? '연동 중...' : '구글 드라이브 연결'}</span>
              </button>
            )}
        ...
        {/* Security Banner (Subtle & Compact) */}
        <div className="bg-[#121318]/70 border border-[#2e3142] rounded px-3 py-1.5 text-xs flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 text-slate-300">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="text-[0.6875rem] text-slate-300">
              <strong className="text-emerald-400 font-medium">최소 권한 원칙 적용:</strong> 본 앱에서 열거나 생성한 전용 파일에만 접근 권한을 요청하므로 개인정보가 안전하게 보호됩니다.
            </span>
          </div>
        </div>

        {/* Notice when disconnected (Compact banner to preserve file viewport) */}
        {authStatus !== 'connected' && (
          <div className="p-3 bg-[#121318] border border-indigo-500/30 rounded text-center space-y-1.5 shrink-0">
            <div className="flex items-center justify-center gap-2 text-indigo-400">
              <AlertCircle className="w-4 h-4" />
              <span className="text-xs font-semibold text-slate-200">구글 드라이브가 아직 연동되지 않았습니다</span>
            </div>
            <p className="text-[0.6875rem] text-slate-400 max-w-md mx-auto">
              버튼 한 번으로 안전하게 연동하여 마크다운 및 HTML 문서를 즉시 불러오거나 저장할 수 있습니다.
            </p>
            <button
              type="button"
              onClick={handleOnDemandConnect}
              disabled={isConnecting}
              className="px-3 py-1 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-medium rounded transition flex items-center gap-1.5 mx-auto cursor-pointer shadow-xs"
            >
              <HardDrive className="w-3.5 h-3.5" />
              ...
              {filteredFiles.length === 0 && !isLoading && (
                <div className="py-8 text-center text-slate-500 text-xs">
                  ...
                  <p className="text-[0.6875rem] text-slate-500 mt-1">
                    '드라이브에 저장' 탭에서 현재 작성 중인 문서를 먼저 업로드할 수 있습니다.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between pt-2.5 border-t border-[#2e3142] shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-4 py-1.5 rounded text-xs font-medium bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white transition shadow-xs cursor-pointer flex items-center gap-1.5"
                >
                  <Download className="w-3.5 h-3.5" />
                  <span>에디터로 불러오기</span>
                </button>
        ...
        {activeTab === 'save' && (
          <div className="flex-1 flex flex-col justify-between min-h-0 space-y-4">
            <div className="space-y-3 bg-[#121318] border border-[#2e3142] p-3.5 rounded">
              ...
              <div className="p-2.5 bg-[#16171e] rounded border border-[#2e3142] text-[0.6875rem] text-slate-400 flex items-center justify-between">
          ...
          <div className="flex-1 flex flex-col min-h-0 space-y-3">
            ...
            <div className="bg-[#121318] border border-[#2e3142] rounded px-3 py-2 flex items-center justify-between text-xs shrink-0">
              ...
              <button
                ...
                className="px-2.5 py-1 bg-[#1e202b] hover:bg-[#282a38] text-slate-200 text-xs rounded border border-[#2e3142] transition flex items-center gap-1 cursor-pointer"
              >
            ...
            {isCreatingFolder && (
              ...
              >
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs rounded transition cursor-pointer"
                >
                  생성
                </button>
                <button
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2.5 border-t border-[#2e3142] shrink-0">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded text-xs font-medium bg-indigo-600 hover:bg-indigo-500 text-white transition shadow-xs cursor-pointer flex items-center gap-1.5"
              >
