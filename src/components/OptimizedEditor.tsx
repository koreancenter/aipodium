...
export const OptimizedEditor: React.FC<OptimizedEditorProps> = memo(({
  ...
  const [tableBubblePos, setTableBubblePos] = useState<{ top: number; left: number }>({ top: 0, left: 0 });
  const [isAiTableLoading, setIsAiTableLoading] = useState(false);
  const [isVisualModalOpen, setIsVisualModalOpen] = useState(false);

  // General Text Bubble Menu State
  const [textBubblePos, setTextBubblePos] = useState<{ top: number; left: number }>({ top: 0, left: 0 });
  const [isTextBubbleVisible, setIsTextBubbleVisible] = useState(false);
  ...
  const updateEditorBubbleContext = useCallback((text: string, selStart: number, selEnd?: number) => {
    ...
    const hasSel = end > selStart;
    const textarea = refToUse.current;

    // 1. Check if cursor/selection is inside a markdown table
    const info = getTableAtCursor(text, selStart);
    setCurrentTableInfo(info);

    if (info) {
      // Hide general text bubble menu when inside table
      setIsTextBubbleVisible(false);
  ...
  const updateTableContext = useCallback((text: string, cursorPos: number) => {
    updateEditorBubbleContext(text, cursorPos, cursorPos);
  }, [updateEditorBubbleContext]);

  const handleEditorScroll = useCallback((e: React.UIEvent<HTMLTextAreaElement>) => {
    const textarea = e.currentTarget;
    updateEditorBubbleContext(localValue, textarea.selectionStart, textarea.selectionEnd);
  }, [localValue, updateEditorBubbleContext]);
  ...
  const handleClearSelectedCells = useCallback(() => {
    ...
    const res = clearTableCells(currentTableInfo, selStart, selEnd);
    const nextVal =
      localValue.slice(0, currentTableInfo.startOffset) +
      res.formatted +
      localValue.slice(currentTableInfo.endOffset);
  ...
  const handleAiTableEdit = useCallback(async (prompt: string) => {
    if (!currentTableInfo) return false;
    setIsAiTableLoading(true);
    try {
      const tableMarkdown = currentTableInfo.lines.join('\n');
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: `다음 마크다운 표를 사용자의 요청에 따라 정확히 수정 또는 요약하세요.
...
[출력 지침]:
1. 오직 가공/요약된 결과 마크다운 표 문자열(| ... |)만 반환할 것.
...
3. 마크다운 표 헤더, 구분선, 행 파이프 문법을 온전히 유지할 것.`,
      ...
      if (!res.ok) {
        throw new Error(`AI 서버 통신 실패 (HTTP ${res.status})`);
      }

      const data = await res.json();
      let reply = data.reply || data.text || '';
      if (!reply.trim()) {
        throw new Error('AI로부터 유효한 표 응답을 수신하지 못했습니다.');
      }

      // Strip code fences if present
      if (reply.startsWith('```markdown')) {
        reply = reply.replace(/^```markdown\s*/i, '').replace(/\s*```$/, '');
      } else if (reply.startsWith('```')) {
        reply = reply.replace(/^```\s*/, '').replace(/\s*```$/, '');
      }

      const trimmedReply = reply.trim();
      if (!trimmedReply.includes('|')) {
        throw new Error('올바른 마크다운 표 형식으로 변환되지 않았습니다.');
      }
      ...
      setLocalValue(nextVal);
      debouncedOnChange(nextVal);
      updateTableContext(nextVal, currentTableInfo.startOffset);
      return true;
    } catch (err: any) {
      console.error('AI Table Edit error:', err);
      // Smart offline calculation fallback if prompt is about totals or averages
      if (prompt.includes('합계') || prompt.includes('평균') || prompt.toLowerCase().includes('total')) {
        try {
          ...
          for (let c = 1; c < cols; c++) {
            ...
            let count = 0;
            for (const r of rows) {
              const val = parseFloat((r[c] || '').replace(/[^0-9.-]+/g, ''));
              if (!isNaN(val)) {
                sum += val;
                count++;
              }
            ...
            if (count > 0) {
              if (prompt.includes('평균')) {
                const avg = sum / count;
                totals[c] = Number.isInteger(avg) ? avg.toString() : avg.toFixed(1);
              } else {
                totals[c] = Number.isInteger(sum) ? sum.toLocaleString() : sum.toFixed(2);
              }
            }
          }
          const newRows = [...rows, totals];
          const formatted = formatMarkdownTable(currentTableInfo.headers, currentTableInfo.alignments, newRows);
          const nextVal =
            localValue.slice(0, currentTableInfo.startOffset) +
            formatted +
            localValue.slice(currentTableInfo.endOffset);
      ...
      throw new Error(err?.message || 'AI 표 가공 실패: 설정에서 API 키를 확인해 주세요.');
    } finally {
      setIsAiTableLoading(false);
    }
  }, [currentTableInfo, localValue, debouncedOnChange, updateTableContext]);

  // General Text Bubble Menu Actions
  const handleApplyTextFormat = useCallback((formatType: TextFormatAction) => {
    const textarea = refToUse.current;
    if (!textarea) return;
    ...
    // 1. Inline formats: bold, italic, strikethrough, code, link, codeblock
    if (formatType === 'bold') {
      let replacement = '';
      let newStart = start;
      let newEnd = end;
      ...
      setTimeout(() => {
        if (textarea) {
          ...
          updateEditorBubbleContext(nextVal, newStart, newEnd);
        }
      }, 20);
      return;
    }
    ...
    if (formatType === 'link') {
      const inner = selected || '링크 텍스트';
      const replacement = `[${inner}](https://)`;
      const newStart = start + 1;
      const newEnd = start + 1 + inner.length;
      const nextVal = val.slice(0, start) + replacement + val.slice(end);
      setLocalValue(nextVal);
      debouncedOnChange(nextVal);
      setTimeout(() => {
    ...
    if (formatType === 'codeblock') {
      const inner = selected || '// 코드를 작성하세요';
      ...
      setTimeout(() => {
        if (textarea) {
          ...
          updateEditorBubbleContext(nextVal, start + 14, start + 14 + inner.length);
        }
      }, 20);
      return;
    }

    // 2. Block/Line formats: h1, h2, h3, bullet, number, task, quote
    ...
    if (formatType === 'h1' || formatType === 'h2' || formatType === 'h3') {
      ...
      newLines = lines.map((line) => {
        ...
        const clean = line.replace(headingPrefixRegex, '').replace(listPrefixRegex, '').trimStart();
        if (line.trim().startsWith(prefix)) {
          return `${indent}${clean}`;
        }
        return `${indent}${prefix}${clean || '제목'}`;
      });
    } else if (formatType === 'bullet') {
      const allAreBullets = lines.filter((l) => l.trim().length > 0).every((l) => /^(\s*)[-*+]\s+/.test(l));
      newLines = lines.map((line) => {
        if (!line.trim() && lines.length > 1) return line;
        const indentMatch = line.match(/^(\s*)/);
        const indent = indentMatch ? indentMatch[1] : '';
        const clean = line.replace(listPrefixRegex, '').replace(headingPrefixRegex, '').trimStart();
        if (allAreBullets) return `${indent}${clean}`;
        return `${indent}- ${clean || '목록 항목'}`;
      });
      ...
      newLines = lines.map((line) => {
        ...
        const indent = indentMatch ? indentMatch[1] : '';
        const clean = line.replace(listPrefixRegex, '').replace(headingPrefixRegex, '').trimStart();
        if (allAreNumbers) return `${indent}${clean}`;
        const res = `${indent}${counter}. ${clean || '목록 항목'}`;
        counter++;
        return res;
      });
    } else if (formatType === 'task') {
      const allAreTasks = lines.filter((l) => l.trim().length > 0).every((l) => /^(\s*)(?:[-*+]\s+)?\[[ xX]\]\s+/.test(l));
      newLines = lines.map((line) => {
        if (!line.trim() && lines.length > 1) return line;
        const indentMatch = line.match(/^(\s*)/);
        const indent = indentMatch ? indentMatch[1] : '';
        const clean = line.replace(listPrefixRegex, '').replace(headingPrefixRegex, '').trimStart();
        if (allAreTasks) return `${indent}${clean}`;
        return `${indent}- [ ] ${clean || '할 일 항목'}`;
      });
      ...
      newLines = lines.map((line) => {
        ...
        const indent = indentMatch ? indentMatch[1] : '';
        const clean = line.replace(listPrefixRegex, '').replace(headingPrefixRegex, '').trimStart();
        if (allAreQuotes) return `${indent}${clean}`;
        return `${indent}> ${clean || '인용문'}`;
      });
    }

    const replacedBlock = newLines.join('\n');
    const nextVal = val.slice(0, lineStart) + replacedBlock + val.slice(lineEnd);
    setLocalValue(nextVal);
    debouncedOnChange(nextVal);
    setTimeout(() => {
      if (textarea) {
        textarea.focus();
        const newSelEnd = lineStart + replacedBlock.length;
        textarea.setSelectionRange(lineStart, newSelEnd);
        updateEditorBubbleContext(nextVal, lineStart, newSelEnd);
      }
    }, 20);
  }, [localValue, debouncedOnChange, refToUse, updateEditorBubbleContext]);

  const handleAiTextEdit = useCallback(async (prompt: string) => {
    const textarea = refToUse.current;
    ...
    if (hasSel) {
      targetText = val.slice(start, end);
      replaceStart = start;
      replaceEnd = end;
    } else {
      let pStart = val.lastIndexOf('\n\n', start - 1);
      if (pStart === -1) pStart = val.lastIndexOf('\n', start - 1);
      pStart = pStart === -1 ? 0 : pStart + 1;

      let pEnd = val.indexOf('\n\n', end);
      if (pEnd === -1) pEnd = val.indexOf('\n', end);
      if (pEnd === -1) pEnd = val.length;

      targetText = val.slice(pStart, pEnd).trim();
      replaceStart = pStart;
      replaceEnd = pEnd;
    }

    if (!targetText) {
      throw new Error('가공할 대상 텍스트가 없습니다. 텍스트를 선택하거나 작성해 주세요.');
    }
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: `다음 텍스트를 사용자의 요청에 따라 정확하게 수정, 교정, 요약 또는 변환하세요.
...
3. 마크다운 서식이 요청된 경우 올바른 마크다운 문법을 적용할 것.`,
      ...
      let reply = data.reply || data.text || '';
      if (!reply.trim()) {
        throw new Error('AI로부터 응답을 수신하지 못했습니다.');
      }

      if (reply.startsWith('```markdown') && reply.endsWith('```')) {
        reply = reply.replace(/^```markdown\s*/i, '').replace(/\s*```$/, '');
      } else if (reply.startsWith('```') && reply.endsWith('```') && !prompt.includes('코드') && !prompt.includes('code')) {
        reply = reply.replace(/^```\s*/, '').replace(/\s*```$/, '');
      }

      const trimmedReply = reply.trim();
      const nextVal = val.slice(0, replaceStart) + trimmedReply + val.slice(replaceEnd);
      setLocalValue(nextVal);
      debouncedOnChange(nextVal);
      ...
      setTimeout(() => {
        if (textarea) {
          textarea.focus();
          const newEnd = replaceStart + trimmedReply.length;
          textarea.setSelectionRange(replaceStart, newEnd);
          updateEditorBubbleContext(nextVal, replaceStart, newEnd);
        }
      }, 20);
      return true;
    } finally {
    ...
    if (hasSel) {
      ...
      setTimeout(() => {
        if (textarea) {
          ...
          updateEditorBubbleContext(nextVal, start, start + cleaned.length);
        }
      }, 20);
    } else {
      const lineStart = val.lastIndexOf('\n', start - 1) + 1;
      ...
      const cleaned = currentLine.trim();
      const nextVal = val.slice(0, lineStart) + cleaned + val.slice(lineEnd);
      setLocalValue(nextVal);
      debouncedOnChange(nextVal);
      setTimeout(() => {
        if (textarea) {
          ...
          updateEditorBubbleContext(nextVal, lineStart, lineStart + cleaned.length);
        }
      }, 20);
    }
  }, [localValue, debouncedOnChange, refToUse, updateEditorBubbleContext]);
  ...
  const handleBlur = () => {
    ...
    // Delay closing text bubble menu if active element is outside editor/menus
    setTimeout(() => {
      const activeEl = document.activeElement;
      const textMenu = document.getElementById('text-floating-bubble-menu');
      const tableMenu = document.getElementById('table-floating-bubble-menu');
      if (textMenu?.contains(activeEl) || tableMenu?.contains(activeEl)) {
        return;
      }
      setIsTextBubbleVisible(false);
    }, 250);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // Escape closes text bubble menu
    if (key === 'Escape') {
    ...
    // Hide general text bubble menu while actively typing regular text without selection
    if (!e.ctrlKey && !e.metaKey && !e.altKey && key.length === 1 && selectionStart === selectionEnd) {
      if (!currentTableInfo) {
        setIsTextBubbleVisible(false);
        if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
        typingTimeoutRef.current = setTimeout(() => {
          if (refToUse.current) {
            updateEditorBubbleContext(refToUse.current.value, refToUse.current.selectionStart, refToUse.current.selectionEnd);
          }
        }, 400);
      }
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Floating Inline Table Bubble Menu */}
          {currentTableInfo && (
            <TableFloatingBubbleMenu
              tableInfo={currentTableInfo}
              position={tableBubblePos}
              ...
              onAiEditTable={handleAiTableEdit}
              isAiLoading={isAiTableLoading}
            />
          )}

          {/* Floating Inline General Text Bubble Menu */}
          {!currentTableInfo && isTextBubbleVisible && (
            <TextFloatingBubbleMenu
