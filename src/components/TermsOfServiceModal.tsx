      ...
      <div className="bg-[#0f172a] border border-slate-700/80 rounded-2xl shadow-2xl flex flex-col w-full max-w-2xl max-h-[88vh] overflow-hidden text-slate-100 font-sans">
        ...
        <div className="flex items-center justify-between px-5 py-4 bg-[#0b1120] border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="terms-title" className="text-sm sm:text-base font-bold text-slate-100 tracking-tight">
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 text-xs text-slate-300 leading-relaxed bg-[#0f172a]">
          ...
          <div className="p-3.5 rounded-xl bg-amber-950/20 border border-amber-500/20 text-amber-200 text-xs leading-relaxed flex items-start gap-2.5">
          ...
          <div className="space-y-4">
            ...
            <section className="space-y-1.5 p-3.5 rounded-xl bg-slate-900/80 border border-slate-800/80">
              ...
              <ul className="list-disc list-inside space-y-1 pl-4 text-slate-300">
                <li>
                  <strong>{isKr ? '"서비스"라 함은' : '"Service" refers to'}</strong>{' '}
                  {isKr
                    ? 'aipodium.net을 통해 제공되는 Local-First 연구 및 문서 작성 소프트웨어를 의미합니다.'
                    : 'the Local-First research and compilation workspace delivered through the aipodium.net portal.'}
                </li>
                <li>
                  <strong>{isKr ? '"BYOK(Bring Your Own Key)"라 함은' : '"BYOK" refers to'}</strong>{' '}
                  {isKr
                    ? '이용자가 직접 발급받은 외부 AI 서비스의 API Key를 등록하여 사용하는 방식을 의미합니다.'
            ...
            <section className="space-y-1.5 p-3.5 rounded-xl bg-slate-900/80 border border-slate-800/80">
              ...
              <ol className="list-decimal list-inside space-y-1.5 pl-4 text-slate-300">
                ...
                <li>
                  <strong>{isKr ? '기기 변경, 브라우저 캐시 삭제, 마스터 PIN 및 복구 키 분실 등으로 발생하는 데이터 손실의 책임은 이용자 본인에게 있습니다.' : 'Data losses originating from machine migrations, hardware failure, manual browser cache clearing, or the misplacement of access credentials (PIN / Recovery Keys) reside solely and fully with the user.'}</strong>
                </li>
              </ol>
            </section>
            ...
            <section className="space-y-1.5 p-3.5 rounded-xl bg-slate-900/80 border border-slate-800/80">
              <h3 className="font-bold text-slate-100 text-xs sm:text-sm">
                {isKr ? '제4조 (BYOK 및 외부 API 이용)' : 'Article 4 (BYOK & API Operations)'}
              </h3>
              <ol className="list-decimal list-inside space-y-1.5 pl-4 text-slate-300">
                <li>{isKr ? '이용자가 등록한 API Key의 사용에 따라 발생하는 외부 AI 서비스(OpenAI, Google 등)의 이용 요금은 이용자 본인이 부담합니다.' : 'Billing costs, subscription thresholds, and tokens spent with external AI companies (OpenAI, Google, Anthropic, etc.) due to local actions are fully paid by the user.'}</li>
                <li>{isKr ? '서비스는 API Key 유출을 방지하기 위해 로컬 암호화를 적용하나, 이용자 기기의 보안 관리 소홀로 인한 키 유출에 대해서는 책임을 지지 않습니다.' : 'While the workspace enforces local cryptographic standards to safeguard registered keys, any leakage arising from client hardware intrusion or insecure credential storage falls beyond our liability.'}</li>
              </ol>
            </section>
            ...
            <section className="space-y-1.5 p-3.5 rounded-xl bg-slate-900/80 border border-slate-800/80">
              ...
              <ol className="list-decimal list-inside space-y-1.5 pl-4 text-slate-300">
                <li>{isKr ? '본 서비스는 Beta v1.0 버전으로 제공되며, 성능 개선을 위해 사전 고지 없이 업데이트될 수 있습니다.' : 'The platform is distributed as a Beta v1.0 software. Features, schemas, and properties may alter or adapt over time to enhance research metrics.'}</li>
                <li><strong>{isKr ? 'AI가 생성한 결과물(인용, 요약 등)의 학술적/법적 검증 책임은 이용자에게 있습니다.' : 'Responsibility for citation verification, thesis validation, plagiarism prevention, and compliance with institutional reviews regarding AI-assisted materials remains strictly with the user.'}</strong></li>
              </ol>
            </section>
          </div>
        </div>
